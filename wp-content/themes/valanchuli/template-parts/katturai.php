<h5 class="row p-2 mt-4 text-highlight-color fw-bold bg-primary-color">🔥 கட்டுரை</h5>
<?php
...
?>
    ...
    $katturai_count = $stories->found_posts;
    ...
    ?>
        ...
        <?php while ($stories->have_posts()) {
            ...
            ?>
                <div class="col-12 col-sm-4 col-md-3 col-xl-2 col-xxl-2 px-5 px-sm-2 p-md-0 d-flex align-items-center justify-content-center text-primary-color katturai-card">
        ...
        <div class="text-center mt-4">
            <button id="katturai-toggle-read-btn" class="btn btn-primary text-highlight-color d-none">Read More</button>
        </div>
    <?php } else {
...
document.addEventListener('DOMContentLoaded', () => {
    const katturaiCards = document.querySelectorAll('.katturai-card');
    const readMoreBtn = document.getElementById('katturai-toggle-read-btn');
    const totalTrendingCount = <?php echo $katturai_count; ?>;
    let expanded = false;
    ...
    function limitCards() {
        ...
        katturaiCards.forEach((card, index) => {
            if (index < visibleLimit) {
                card.classList.remove('d-none', 'extra-katturai-story');
            } else {
                card.classList.add('d-none', 'extra-katturai-story');
            }
        });
    ...
    readMoreBtn.addEventListener('click', () => {
        if (!expanded) {
            katturaiCards.forEach(card => card.classList.remove('d-none'));
